...
namespace MeshPlugin
{
    ...
    //
    // Раньше LIREXPORT делал всё одним методом на 796 строк: спрашивал параметры,
    ...
    public partial class Commands
    {
        internal ExportResult BuildExportCore(ExportInput input)
        {
            ...
            // Делать это в LIRBUILD бесполезно — вдоль оси стены линий сетки нет
            ...
            // пилона (это обеспечивает отпечаток в LIRBUILD).
            ...
            if (slabElemCount == 0)
            {
                res.Error = "\nНе найдено ни одной замкнутой ячейки сетки — сначала постройте сетку (LIRBUILD).\n";
                return res;
            }
