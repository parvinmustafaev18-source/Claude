...
| Функция | Что делает |
|---|---|
| `SegmentCrossingPoint(...)` | Х-пересечение внутренностей; ближе 0.5 мм к концу = узловое касание, не в счёт |
| `SplitSegmentsAtIntersections(segs, out n)` | режет все Х-пересечения; пары ищет через `SpatialGrid` |

Обе функции жили в Quality.cs рядом с командой MESHQUALITY; команду убрали
23.08.2026, функции переехали в Geometry.cs — их зовёт `BuildMeshCore`
(жёсткое правило 6) и экспорт.

Порядок вызовов (dedup → пересечения → узлы → dedup) обязателен: планарный граф
строится по общим узлам, и пересечение без узла даёт грань, накрывающую чужие
линии.
...
- `SpatialGrid(cellSize)` (стр. 12): `Add(index, point)`, `QueryRadius(p, r)`.
  ...
  заметно.
- `BboxIndex(cellSize)`: индексирует не точки, а ГАБАРИТЫ (`AddSegment`,
  `AddPolygon`, `Query(minX,minY,maxX,maxY)`). Держит главный цикл ячеек: без него
  на каждую ячейку перебирались все стены, пилоны и отверстия плана. Ответ идёт
  **по возрастанию индекса** — порядок обязателен, `SplitPolygonByWalls` режет
  ячейку последовательно и от перестановки стен форма кусков меняется. Объект,
  габарит которого накрывает больше 1024 бакетов (косая линия через весь план),
  в бакеты не кладётся и возвращается на каждый запрос. Буфер ответа общий:
  содержимое живёт до следующего запроса.
